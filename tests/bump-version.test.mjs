...
function makeVersionFixture() {
  ...
  writeJson(path.join(root, "plugins", "qwen", ".claude-plugin", "plugin.json"), {
    name: "qwen",
    version: "1.0.2"
  });
...
test("bump-version updates every release manifest", () => {
  ...
  assert.equal(readJson(path.join(root, "plugins", "qwen", ".claude-plugin", "plugin.json")).version, "1.2.3");
...
test("bump-version check mode reports stale metadata", () => {
  ...
  assert.match(result.stderr, /plugins\/qwen\/\.claude-plugin\/plugin\.json version/);
