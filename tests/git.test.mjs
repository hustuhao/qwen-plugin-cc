...
test("collectReviewContext keeps only untracked file summaries in lightweight working tree context", () => {
  const cwd = makeTempDir();
  initGitRepo(cwd);
  ...
  assert.match(context.content, /new-risk\.js/);
  assert.doesNotMatch(context.content, /UNTRACKED_RISK_MARKER/);
  assert.match(context.content, /content omitted in lightweight mode/i);
});
