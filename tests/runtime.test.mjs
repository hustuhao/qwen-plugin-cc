...
import { loadBrokerSession, saveBrokerSession } from "../plugins/qwen/scripts/lib/broker-lifecycle.mjs";
...
test("setup reports ready when fake qwen is installed and authenticated", () => {
  ...
  assert.match(payload.qwen.detail, /ACP runtime available/);
});

test("setup is ready without npm when Qwen is already installed and authenticated", () => {
  const binDir = makeTempDir();
  installFakeQwen(binDir);
...
test("setup trusts app-server API key auth even when login status alone would fail", () => {
  ...
  assert.equal(payload.auth.source, "settings");
  assert.match(payload.auth.detail, /Authenticated via apiKey/);
});

test("setup is ready when the active provider does not require external login", () => {
  const binDir = makeTempDir();
  installFakeQwen(binDir, "provider-no-auth");
  ...
  assert.equal(payload.auth.source, "settings");
  assert.match(payload.auth.detail, /configured and does not require external authentication/i);
});

test("setup treats custom providers with app-server-ready config as ready", () => {
  const binDir = makeTempDir();
  installFakeQwen(binDir, "env-key-provider");
...
test("setup reports not ready when app-server config read fails", () => {
  const binDir = makeTempDir();
  installFakeQwen(binDir, "config-read-fails");
  ...
  assert.equal(payload.auth.source, "settings");
  assert.match(payload.auth.detail, /authentication not configured/i);
});
...
test("task runs without auth preflight so Qwen can refresh an expired session", () => {
  const repo = makeTempDir();
  const binDir = makeTempDir();
  installFakeQwen(binDir, "refreshable-auth");
  initGitRepo(repo);
  fs.writeFileSync(path.join(repo, "README.md"), "hello\n");
  run("git", ["add", "README.md"], { cwd: repo });
  run("git", ["commit", "-m", "init"], { cwd: repo });
...
test("task reports the actual Qwen auth error when the run is rejected", () => {
  const repo = makeTempDir();
  const binDir = makeTempDir();
  installFakeQwen(binDir, "auth-run-fails");
  initGitRepo(repo);
  fs.writeFileSync(path.join(repo, "README.md"), "hello\n");
  run("git", ["add", "README.md"], { cwd: repo });
  run("git", ["commit", "-m", "init"], { cwd: repo });
...
test("adversarial review asks Qwen to inspect larger diffs itself", () => {
  const repo = makeTempDir();
  const binDir = makeTempDir();
  installFakeQwen(binDir);
  initGitRepo(repo);
  ...
  assert.equal(result.status, 0, result.stderr);
  const state = JSON.parse(fs.readFileSync(path.join(binDir, "fake-qwen-state.json"), "utf8"));
...
test("adversarial review prompt hardens JSON-only output instructions", () => {
  const repo = makeTempDir();
  const binDir = makeTempDir();
  installFakeQwen(binDir);
  initGitRepo(repo);
  ...
  assert.equal(result.status, 0, result.stderr);
  const state = JSON.parse(fs.readFileSync(path.join(binDir, "fake-qwen-state.json"), "utf8"));
  assert.match(state.lastTurnStart.prompt, /Do not write any prose before the JSON\./);
  assert.match(state.lastTurnStart.prompt, /Do not wrap the JSON in Markdown fences such as ```json\./);
  assert.match(state.lastTurnStart.prompt, /Your first non-whitespace character must be `\\?{`?\./);
  assert.match(state.lastTurnStart.prompt, /emit exactly one JSON object/i);
  assert.match(state.lastTurnStart.prompt, /no Markdown fences/i);
});

test("adversarial review returns a rendered parse-error report without failing the command when Qwen hits an internal error", () => {
  const repo = makeTempDir();
  const binDir = makeTempDir();
  installFakeQwen(binDir, "adversarial-internal-error");
  initGitRepo(repo);
  fs.mkdirSync(path.join(repo, "src"));
  fs.writeFileSync(path.join(repo, "src", "app.js"), "export const value = items[0];\n");
  run("git", ["add", "src/app.js"], { cwd: repo });
  ...
  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /Qwen did not return valid structured JSON/i);
  assert.match(result.stdout, /Parse error: Internal error/i);
});

test("adversarial review prefers thread protocol when structured output on session/prompt is broken", () => {
  const repo = makeTempDir();
  const binDir = makeTempDir();
  installFakeQwen(binDir, "schema-prefers-thread");
  initGitRepo(repo);
  fs.mkdirSync(path.join(repo, "src"));
  fs.writeFileSync(path.join(repo, "src", "app.js"), "export const value = items[0];\n");
  run("git", ["add", "src/app.js"], { cwd: repo });
  ...
  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /Missing empty-state guard/i);
  const state = JSON.parse(fs.readFileSync(path.join(binDir, "fake-qwen-state.json"), "utf8"));
  assert.equal(state.lastTurnStart.threadId.startsWith("thr_"), true);
});

test("review includes reasoning output when the app server returns it", () => {
  const repo = makeTempDir();
  const binDir = makeTempDir();
  installFakeQwen(binDir, "with-reasoning");
  initGitRepo(repo);
  fs.writeFileSync(path.join(repo, "README.md"), "hello\n");
  run("git", ["add", "README.md"], { cwd: repo });
  run("git", ["commit", "-m", "init"], { cwd: repo });
...
test("task-resume-candidate returns the latest rescue thread from the current session", () => {
  ...
  const result = run("node", [SCRIPT, "task-resume-candidate", "--json"], {
    ...
    env: {
      ...process.env,
      QWEN_COMPANION_SESSION_ID: "sess-current"
    }
  });
...
test("task --resume-last does not resume a task from another Claude session", () => {
  ...
  const otherEnv = {
    ...buildEnv(binDir),
    QWEN_COMPANION_SESSION_ID: "sess-other"
  };
  const currentEnv = {
    ...buildEnv(binDir),
    QWEN_COMPANION_SESSION_ID: "sess-current"
  ...
  assert.match(resume.stderr, /No previous Qwen task session was found for this repository\./);
...
test("session start hook exports the Claude session id and plugin data dir for later commands", () => {
  ...
  assert.equal(
    ...
    `export QWEN_COMPANION_SESSION_ID='sess-current'\nexport CLAUDE_PLUGIN_DATA='${pluginDataDir}'\n`
  );
  ...
  assert.deepEqual(brokerSession.owners.map((owner) => owner.sessionId), ["sess-current"]);
});

test("write task output focuses on the Qwen result without generic follow-up hints", () => {
  const repo = makeTempDir();
  const binDir = makeTempDir();
  installFakeQwen(binDir);
  initGitRepo(repo);
...
test("task still runs when Qwen only supports the thread protocol", () => {
  const repo = makeTempDir();
  const binDir = makeTempDir();
  installFakeQwen(binDir, "thread-protocol");
  initGitRepo(repo);
  fs.writeFileSync(path.join(repo, "README.md"), "hello\n");
  run("git", ["add", "README.md"], { cwd: repo });
  run("git", ["commit", "-m", "init"], { cwd: repo });
...
test("task forwards model selection and reasoning effort to app-server turn/start", () => {
  ...
  const result = run("node", [SCRIPT, "task", "--model", "qwen-plus", "--effort", "low", "diagnose the failing test"], {
    cwd: repo,
    env: buildEnv(binDir)
  });
  ...
  assert.equal(result.status, 0, result.stderr);
  const fakeState = JSON.parse(fs.readFileSync(statePath, "utf8"));
  assert.equal(fakeState.lastTurnStart.model, "qwen-plus");
...
test("task logs subagent reasoning and messages with a subagent prefix", () => {
  const repo = makeTempDir();
  const binDir = makeTempDir();
  installFakeQwen(binDir, "with-subagent");
  initGitRepo(repo);
  fs.writeFileSync(path.join(repo, "README.md"), "hello\n");
  run("git", ["add", "README.md"], { cwd: repo });
  run("git", ["commit", "-m", "init"], { cwd: repo });
...
test("task ignores later subagent messages when choosing the final returned output", () => {
  const repo = makeTempDir();
  const binDir = makeTempDir();
  installFakeQwen(binDir, "with-late-subagent-message");
  initGitRepo(repo);
  fs.writeFileSync(path.join(repo, "README.md"), "hello\n");
  run("git", ["add", "README.md"], { cwd: repo });
  run("git", ["commit", "-m", "init"], { cwd: repo });
...
test("task can finish after subagent work even if the parent turn/completed event is missing", () => {
  const repo = makeTempDir();
  const binDir = makeTempDir();
  installFakeQwen(binDir, "with-subagent-no-main-turn-completed");
  initGitRepo(repo);
  fs.writeFileSync(path.join(repo, "README.md"), "hello\n");
  run("git", ["add", "README.md"], { cwd: repo });
  run("git", ["commit", "-m", "init"], { cwd: repo });
...
test("task using the shared broker still completes when Qwen spawns subagents", () => {
  const repo = makeTempDir();
  const binDir = makeTempDir();
  installFakeQwen(binDir, "with-subagent");
  initGitRepo(repo);
  ...
  const latestJob = state.jobs.find((job) => job.jobClass === "task");
  assert.equal(latestJob.transport, "shared");
  assert.equal(latestJob.runtimeSessionId, latestJob.threadId);
});

test("task falls back to direct ACP startup when a saved shared broker endpoint is stale", () => {
  const repo = makeTempDir();
  const binDir = makeTempDir();
  installFakeQwen(binDir, "with-subagent");
  initGitRepo(repo);
  ...
  const state = JSON.parse(fs.readFileSync(path.join(resolveStateDir(repo), "state.json"), "utf8"));
  const latestJob = state.jobs.find((job) => job.jobClass === "task");
  assert.equal(latestJob.transport, "direct");
  assert.equal(typeof latestJob.fallbackReason, "string");
  assert.match(latestJob.fallbackReason, /shared qwen runtime is unavailable|connect|broker/i);
});

test("task --background enqueues a detached worker and exposes per-job status", async () => {
  const repo = makeTempDir();
  const binDir = makeTempDir();
  installFakeQwen(binDir, "slow-task");
  initGitRepo(repo);
  fs.writeFileSync(path.join(repo, "README.md"), "hello\n");
  run("git", ["add", "README.md"], { cwd: repo });
  run("git", ["commit", "-m", "init"], { cwd: repo });
...
test("review rejects focus text because it is native-review only", () => {
  ...
  assert.match(result.stderr, /\/qwen:adversarial-review focus on auth/i);
});
...
test("review accepts --background while still running as a tracked review job", () => {
  ...
  assert.match(launchPayload.qwen.stdout, /No material issues found/);
  ...
  assert.equal(status.status, 0, status.stderr);
  assert.match(status.stdout, /# Qwen Status/);
  assert.match(status.stdout, /Qwen Review/);
...
test("status shows phases, hints, and the latest finished job", () => {
  ...
  fs.writeFileSync(
    logFile,
    [
      "[2026-03-18T15:30:00.000Z] Starting Qwen Review.",
  ...
  assert.match(result.stdout, /\| Job \| Kind \| Status \| Phase \| Transport \| Elapsed \| Qwen Session ID \| Summary \| Actions \|/);
  assert.match(result.stdout, /\| review-live \| review \| running \| reviewing \| .* \| thr_1 \| Review working tree diff \|/);
  assert.match(result.stdout, /`\/qwen:status review-live`<br>`\/qwen:cancel review-live`/);
  ...
  assert.match(result.stdout, /Qwen session ID: thr_1/);
  assert.match(result.stdout, /Resume in Qwen: qwen --resume thr_1/);
  ...
  assert.match(result.stdout, /Qwen session ID: thr_done/);
  assert.match(result.stdout, /Resume in Qwen: qwen --resume thr_done/);
});
...
test("status preserves adversarial review kind labels", () => {
  ...
  assert.match(result.stdout, /- review-adv \| completed \| adversarial-review \| Qwen Adversarial Review/);
  assert.match(result.stdout, /Qwen session ID: thr_adv_live/);
  assert.match(result.stdout, /Qwen session ID: thr_adv_done/);
});
...
test("status --wait times out cleanly when a job is still active", () => {
  ...
  fs.writeFileSync(logFile, "[2026-03-18T15:30:00.000Z] Starting Qwen Task.\n", "utf8");
  fs.writeFileSync(
...
test("result returns the stored output for the latest finished job by default", () => {
  ...
  assert.equal(
    result.stdout,
    "Reviewed uncommitted changes.\nNo material issues found.\n\nQwen session ID: thr_review_finished\nResume in Qwen: qwen --resume thr_review_finished\n"
  );
});
...
test("result without a job id prefers the latest finished job from the current Claude session", () => {
  ...
  assert.equal(
    result.stdout,
    "Current session output.\n\nQwen session ID: thr_current\nResume in Qwen: qwen --resume thr_current\n"
  );
});

test("result for a finished write-capable task returns the raw Qwen final response", () => {
  const repo = makeTempDir();
  const binDir = makeTempDir();
  installFakeQwen(binDir);
  initGitRepo(repo);
  ...
  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /^Handled the requested task\.\nTask prompt accepted\.\n/);
  assert.match(result.stdout, /Qwen session ID: thr_[a-z0-9]+/i);
  assert.match(result.stdout, /Resume in Qwen: qwen --resume thr_[a-z0-9]+/i);
});
...
test("cancel without a job id ignores active jobs from other Claude sessions", () => {
  ...
  assert.match(cancel.stderr, /No active Qwen jobs to cancel for this session\./);
...
test("cancel sends turn interrupt to the shared app-server before killing a brokered task", async () => {
  const repo = makeTempDir();
  const binDir = makeTempDir();
  const fakeStatePath = path.join(binDir, "fake-qwen-state.json");
  installFakeQwen(binDir, "interruptible-slow-task");
  initGitRepo(repo);
  fs.writeFileSync(path.join(repo, "README.md"), "hello\n");
  run("git", ["add", "README.md"], { cwd: repo });
  run("git", ["commit", "-m", "init"], { cwd: repo });
  ...
  assert.equal(Object.hasOwn(cancelPayload, "transport"), true);
  assert.equal(cancelPayload.runtimeSessionId, runningJob.threadId);
...
test("session end releases only the current lease and does not shut down a broker still used by another session", () => {
  const repo = makeTempDir();
  initGitRepo(repo);
  const freshSeenAt = new Date(Date.now() - 1000).toISOString();

  saveBrokerSession(repo, {
    endpoint: "unix:/tmp/fake-broker.sock",
    pidFile: path.join(repo, "broker.pid"),
    logFile: path.join(repo, "broker.log"),
    sessionDir: repo,
    pid: 12345,
    owners: [
      { sessionId: "sess-a", lastSeenAt: freshSeenAt },
      { sessionId: "sess-b", lastSeenAt: freshSeenAt }
    ]
  });
  ...
  const result = run("node", [SESSION_HOOK, "SessionEnd"], {
    ...
    env: {
      ...process.env,
      QWEN_COMPANION_SESSION_ID: "sess-a"
    },
    input: JSON.stringify({
      hook_event_name: "SessionEnd",
      session_id: "sess-a",
  ...
  assert.equal(result.status, 0, result.stderr);
  const brokerSession = loadBrokerSession(repo);
  assert.equal(brokerSession.endpoint, "unix:/tmp/fake-broker.sock");
  assert.deepEqual(brokerSession.owners.map((owner) => owner.sessionId), ["sess-b"]);
});
...
test("stop hook runs a stop-time review task and blocks on findings when the review gate is enabled", () => {
  ...
  assert.match(blockedPayload.reason, /Qwen stop-time review found issues that still need fixes/i);
  ...
  const status = run("node", [SCRIPT, "status"], {
    ...
    env: {
      ...buildEnv(binDir),
      QWEN_COMPANION_SESSION_ID: "sess-stop-review"
    }
  });
  assert.equal(status.status, 0, status.stderr);
  assert.match(status.stdout, /Qwen Stop Gate Review/);
});

test("stop hook exits promptly even when the ACP process does not close on its own", () => {
  const repo = makeTempDir();
  const binDir = makeTempDir();
  installFakeQwen(binDir, "hang-on-close");
  initGitRepo(repo);
  fs.writeFileSync(path.join(repo, "README.md"), "hello\n");
  run("git", ["add", "README.md"], { cwd: repo });
  run("git", ["commit", "-m", "init"], { cwd: repo });
  ...
  assert.equal(blocked.status, 0, blocked.stderr);
  assert.ok(elapsedMs < 8500, `stop hook should not hang after receiving the final review output (elapsed ${elapsedMs}ms)`);
...
test("stop hook falls back to direct startup when the saved shared broker is unavailable", () => {
  const repo = makeTempDir();
  const binDir = makeTempDir();
  const fakeStatePath = path.join(binDir, "fake-qwen-state.json");
  installFakeQwen(binDir);
  ...
  saveBrokerSession(repo, {
    endpoint: "unix:/tmp/qwen-plugin-missing-stop-hook.sock",
    owners: [{ sessionId: "sess-stop-review", lastSeenAt: new Date(Date.now() - 1000).toISOString() }]
  });
...
test("stop hook logs running tasks to stderr without blocking when the review gate is disabled", () => {
  ...
  assert.match(blocked.stderr, /Qwen task task-live is still running/i);
  assert.match(blocked.stderr, /\/qwen:status/i);
  assert.match(blocked.stderr, /\/qwen:cancel task-live/i);
});

test("stop hook allows the stop when the review gate is enabled and the stop-time review task is clean", () => {
  const repo = makeTempDir();
  const binDir = makeTempDir();
  installFakeQwen(binDir, "adversarial-clean");
  initGitRepo(repo);
  fs.writeFileSync(path.join(repo, "README.md"), "hello\n");
  run("git", ["add", "README.md"], { cwd: repo });
  run("git", ["commit", "-m", "init"], { cwd: repo });
...
test("stop hook exits immediately when Claude is already continuing due to a previous stop hook block", () => {
  const repo = makeTempDir();
  const binDir = makeTempDir();
  const fakeStatePath = path.join(binDir, "fake-qwen-state.json");
  installFakeQwen(binDir);
  ...
  assert.ok(!fs.existsSync(fakeStatePath), "stop hook should not invoke Qwen while stop_hook_active=true");
});

test("stop hook does not block when Qwen is unavailable even if the review gate is enabled", () => {
  const repo = makeTempDir();
  initGitRepo(repo);
  fs.writeFileSync(path.join(repo, "README.md"), "hello\n");
  run("git", ["add", "README.md"], { cwd: repo });
  ...
  assert.equal(allowed.status, 0, allowed.stderr);
  assert.equal(allowed.stdout.trim(), "");
  assert.match(allowed.stderr, /Qwen is not set up for the review gate/i);
  assert.match(allowed.stderr, /Run \/qwen:setup/i);
});
...
test("stop hook runs the actual task when auth status looks stale", () => {
  ...
  assert.equal(allowed.status, 0, allowed.stderr);
  assert.doesNotMatch(allowed.stderr, /Qwen is not set up for the review gate/i);
...
test("status reports shared session runtime when a lazy broker is active", () => {
  ...
  assert.match(result.stdout, /Transport: shared/);
});
