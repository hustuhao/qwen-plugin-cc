...
function formatUntrackedFileSummary(cwd, relativePath) {
  const absolutePath = path.join(cwd, relativePath);
  let stat;
  try {
    stat = fs.statSync(absolutePath);
  } catch {
    return `- ${relativePath} (skipped: broken symlink or unreadable file)`;
  }
  if (stat.isDirectory()) {
    return `- ${relativePath} (directory)`;
  }
  if (stat.size > MAX_UNTRACKED_BYTES) {
    return `- ${relativePath} (${stat.size} bytes, content omitted)`;
  }
  ...
  return `- ${relativePath} (${stat.size} bytes, text file; content omitted in lightweight mode)`;
}
...
function collectWorkingTreeContext(cwd, state, options = {}) {
  ...
  if (includeDiff) {
    ...
    const untrackedBody = state.untracked.map((file) => formatUntrackedFileSummary(cwd, file)).join("\n");
    parts = [
      formatSection("Git Status", status),
