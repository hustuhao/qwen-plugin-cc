...
function appendActiveJobsTable(lines, jobs) {
  ...
  lines.push("| Job | Kind | Status | Phase | Transport | Elapsed | Qwen Session ID | Summary | Actions |");
  lines.push("| --- | --- | --- | --- | --- | --- | --- | --- | --- |");
  for (const job of jobs) {
    ...
    lines.push(
      `| ${escapeMarkdownCell(job.id)} | ${escapeMarkdownCell(job.kindLabel)} | ${escapeMarkdownCell(job.status)} | ${escapeMarkdownCell(job.phase ?? "")} | ${escapeMarkdownCell(job.transport ?? "")} | ${escapeMarkdownCell(job.elapsed ?? "")} | ${escapeMarkdownCell(job.runtimeSessionId ?? job.threadId ?? "")} | ${escapeMarkdownCell(job.summary ?? "")} | ${actions.map((action) => `\`${action}\``).join("<br>")} |`
    );
  }
}
...
function pushJobDetails(lines, job, options = {}) {
  ...
  if (job.transport) {
    lines.push(`  Transport: ${job.transport}`);
  }
  ...
  if (job.runtimeSessionId ?? job.threadId) {
    lines.push(`  Qwen session ID: ${job.runtimeSessionId ?? job.threadId}`);
  }
  ...
  if (job.fallbackReason) {
    lines.push(`  Fallback reason: ${job.fallbackReason}`);
  }
  ...
  if (job.status !== "queued" && job.status !== "running" && job.jobClass === "task" && job.write && options.showReviewHint) {
    lines.push("  Review changes: /qwen:review --wait");
    lines.push("  Stricter review: /qwen:adversarial-review --wait");
  }
...
export function renderStatusReport(report) {
  ...
  if (report.needsReview) {
    ...
    lines.push("Ending the session will trigger a fresh Qwen adversarial review and block if it finds issues.");
  }
