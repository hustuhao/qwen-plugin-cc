...
function printUsage() {
  console.log(
    [
      ...
      "  node scripts/qwen-companion.mjs task [--background] [--write] [--resume-last|--resume|--fresh] [--model <model>] [--effort <none|minimal|low|medium|high|xhigh>] [prompt]",
      "  node scripts/qwen-companion.mjs status [job-id] [--all] [--json]",
...
async function buildSetupReport(cwd, actionsTaken = []) {
  ...
  const authStatus = await getQwenAuthStatus(cwd);
  const config = getConfig(workspaceRoot);

  const nextSteps = [];
  if (!qwenStatus.available) {
    nextSteps.push("Install Qwen Code with the official installer: https://help.aliyun.com/zh/model-studio/qwen-code");
  }
  if (qwenStatus.available && !authStatus.loggedIn) {
    nextSteps.push("Run `!qwen login` to authenticate.");
    nextSteps.push("If browser login is blocked, retry with `!qwen login --device-auth` or `!qwen login --with-api-key`.");
  }
...
function ensureQwenAvailable(cwd) {
  ...
  if (!availability.available) {
    throw new Error("Qwen Code CLI is not installed or is missing required runtime support. Install it from https://help.aliyun.com/zh/model-studio/qwen-code, then rerun `/qwen:setup`.");
  }
}
...
function validateNativeReviewRequest(target, focusText) {
  if (focusText.trim()) {
    throw new Error(
      `\`/qwen:review\` now maps directly to the built-in reviewer and does not support custom focus text. Retry with \`/qwen:adversarial-review ${focusText.trim()}\` for focused review instructions.`
    );
  }

  const nativeTarget = buildNativeReviewTarget(target);
  if (!nativeTarget) {
    throw new Error("This `/qwen:review` target is not supported by the built-in reviewer. Retry with `/qwen:adversarial-review` for custom targeting.");
  }
...
async function executeReviewRun(request) {
  ...
  if (reviewName === "Review") {
    ...
    const result = await runAcpReview(request.cwd, {
      target: reviewTarget,
      model: request.model,
  ...
  const result = await runAcpPrompt(context.repoRoot, {
    prompt,
    model: request.model,
    sandbox: "read-only",
  ...
  return {
    ...
    brokerEndpoint: result.brokerEndpoint,
...
async function executeTaskRun(request) {
  ...
  if (request.resumeLast) {
    ...
    if (!latestSession) {
      throw new Error("No previous Qwen task session was found for this repository.");
    }
...
function buildTaskRunMetadata({ prompt, resumeLast = false }) {
  if (!resumeLast && String(prompt ?? "").includes(STOP_REVIEW_TASK_MARKER)) {
    return {
      ...
      summary: "Stop-gate review of previous Claude prompt"
    };
  }
    ...
    candidate:
      candidate == null
        ...
        : {
            ...
            threadId: candidate.threadId ?? candidate.sessionId ?? null,
...
async function handleCancel(argv) {
  ...
  const qwenSessionId = existing.threadId ?? job.threadId ?? existing.sessionId ?? job.sessionId ?? null;
  const promptId = existing.turnId ?? job.turnId ?? existing.promptId ?? job.promptId ?? null;
  const transport = existing.transport ?? job.transport ?? null;
  const brokerEndpoint = existing.brokerEndpoint ?? job.brokerEndpoint ?? null;

  const interrupt = await interruptAcpSession(cwd, { sessionId: qwenSessionId, promptId, transport, brokerEndpoint });
  if (interrupt.attempted) {
    appendLogLine(
      job.logFile,
      interrupt.interrupted
        ? `Requested Qwen prompt interrupt for ${promptId} on ${qwenSessionId}.`
        : `Qwen prompt interrupt failed${interrupt.detail ? `: ${interrupt.detail}` : "."}`
    );
  }
  ...
  const nextJob = {
    ...
    phase: "cancelled",
    transport,
    runtimeSessionId: existing.runtimeSessionId ?? job.runtimeSessionId ?? qwenSessionId,
    brokerEndpoint,
    fallbackReason: existing.fallbackReason ?? job.fallbackReason ?? null,
    pid: null,
    completedAt,
  ...
  writeJobFile(workspaceRoot, job.id, {
    ...existing,
    ...nextJob,
    transportHistory: existing.transportHistory ?? job.transportHistory ?? [],
  ...
  const payload = {
    ...
    title: job.title,
    transport,
    runtimeSessionId: nextJob.runtimeSessionId,
    fallbackReason: nextJob.fallbackReason,
