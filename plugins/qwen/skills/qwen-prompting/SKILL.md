---
name: qwen-prompting
description: Internal guidance for composing Qwen prompts for coding, review, diagnosis, and research tasks inside the Qwen Claude Code plugin
...
- Use `task --resume-last` for follow-up instructions on the same Qwen thread. Send only the delta instruction instead of restating the whole prompt unless the direction changed materially.
...
Concrete end-to-end templates live in [references/qwen-prompt-recipes.md](references/qwen-prompt-recipes.md).
Common failure modes to avoid live in [references/qwen-prompt-antipatterns.md](references/qwen-prompt-antipatterns.md).
