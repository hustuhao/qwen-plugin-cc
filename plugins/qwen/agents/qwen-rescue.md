...
- If the user explicitly chose `--background`, run the handoff in the background.
- If the user explicitly chose `--wait`, run the handoff in the foreground.
- If the user did not explicitly choose `--background` or `--wait`, always run the handoff in the foreground.
- Do not infer background execution from task size, complexity, or expected duration.
...
- If the user asks for a concrete model name such as `qwen-max` or `qwen-plus`, pass it through with `--model`.
